...
### 8.10 Scenario content

Added 17 September 2026 with S01-B07 (D-038). It extends the kinds of section 4 and the commands of section 5; large content still never uses this link.

- **Holdings in `health`.** The `d` map of `health` may carry `scn`, the scenario versions the device holds: a list of maps `{id:<scenario id>, ver:<version, unsigned integer>}`. A `health` without `scn`, or with `scn` null, says nothing about holdings. An empty list says the device holds none. A device that knows `scn` sends it in every `health`, and sends a `health` as soon as it is connected.
- **Event kind `content`.** `d` is `{id, ver, st}`, where `st` is `installing`, `installed`, `failed` or `removed`. A `failed` report may add `e`, the reason as text. The kind is journaled, acknowledged and replayed like every other.
- **Command `content_available`.** `a` is `{id, ver, sha, size}`: the scenario version, its manifest hash as 64 lower case hex digits, and the size of the package in bytes. The device answers `res` at once, `ok` true when it takes the announcement, also when it holds or installs that version already, and `ok` false with `e` when it cannot. It does not wait for the download.
- **The download.** The device fetches `GET https://<server>:<port>/api/v1/scenarios/<id>/<ver>/package.zip`, the host and port of its link, with the header `Authorization: Bearer <device_token>`. The answer carries `X-Manifest-SHA256`; a device refuses a package whose header differs from `sha`. A cut download is resumed with `Range: bytes=<received>-`. The device reports `installing` when it starts. It reports `installed` only when the package has `size` bytes, its manifest hash equals `sha`, and it validates; otherwise it reports `failed`. A version the device deletes is reported `removed`.
- **What the server records.** After a batch is stored, the server applies its events in order to what the device holds: a `health` with `scn` makes exactly those versions current and every other version of the device not current; `installed` makes one version current, `removed` makes it not current; `installing` and `failed` are only journaled and logged, `failed` with its reason. A report the server cannot use, such as an id that is not a scenario id or version 0, stays in the journal and changes nothing. A version once held keeps its row with the time it last became current.
- **When the server announces.** A session plays one published scenario version. The server sends `content_available` to every device of the session that does not hold that version: when the version is assigned, when a device joins the session, and, for a device that was offline then, after its first `health` with `scn` on its next connection. This holds while the session is created or running. The answers are logged; a device that is offline is announced to later, a refusal is logged with its reason.
- The reference is simtarget: `--holdings id@version,...` sets versions to report besides the packages in its content directory, `--content-dir` holds what it installs.

### 8.11 The session tells the device what it plays

Added 21 September 2026 with S01-B11 (D-056, D-057, D-059). It fills in what sections 3 and 5 name: `ses` in `welcome` and `scn` in `session_start`. A device never derives what it plays from the last announcement.

- **`scn` in `welcome`.** `welcome` carries `scn`, the scenario version of the running session of that device: a map `{id:<scenario id>, ver:<version, unsigned integer>}`, or null. It is null when no session of the device runs and when the running session has no published version assigned. `ses` and `scn` belong together: `scn` is the version that `ses` plays. A device that does not know `scn` ignores it, as section 2 asks.
- **`session_start`.** `a` is `{ses:<session id>, scn:{id, ver}}`, the same shape as in `welcome`. The server sends it to every device of the session when the session starts, to a device that connects while the session runs, right after its `welcome`, and to a device that is put into a running session. A device answers `ok` true when it plays that version; `ok` false with `e` when it cannot, for example because it does not hold the version.
- **`session_stop`.** `a` is `{ses}`, as section 5 says. The server sends it to every device of the session when the session stops and then forgets what it knew about that session.
